...
object Day20 {
  ...
  def shiftAll(original: List[Long], shifted: ArrayBuffer[(Long, Int)]) = {
  ...
  def elementsAfterZero(shifted: ArrayBuffer[(Long, Int)]): Seq[Long] = {
    val indexOf0 = shifted.map(_._1).indexOf(0)
    Seq(1000, 2000, 3000).map { i =>
      val iFind = (indexOf0 + i) % shifted.length
      shifted(iFind)._1
    }
  }
  ...
  def day20Part1 = printSolution {   // 6387
    ...
    val shifted = ArrayBuffer.from(original.zipWithIndex)

    shiftAll(original, shifted)

    val elements = elementsAfterZero(shifted)
    elements.sum
  }
